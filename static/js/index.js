...
const changeProcessingButton = () => {
    ...
    document.getElementById('button-content').innerHTML = changeButtonContent;
}


const changeSearchButton = () => {
    const changeButtonContent = "検索";

    document.getElementById('button-content').innerHTML = changeButtonContent;
}


const changeProcessingSpaceMessage = (name) => {
    const currentlyInprocessContent = `
    ...
    document.getElementById(`${name}-spaces-message`).innerHTML = currentlyInprocessContent;
    document.getElementById(`${name}-spaces-message`).innerHTML = currentlyInprocessContent;
}


const addSpacePosinaga = (name) => {
    const posiNagaCount = `
        <div class="flex">
...
const incrementCountSpacePositive = (name) => {
    document.getElementById(`${name}-positive-count`).value = parseInt(document.getElementById(`${name}-positive-count`).value) + 1;
    document.getElementById(`show-${name}-positive-count`).textContent = document.getElementById(`${name}-positive-count`).value;
}


const incrementCountSpaceNegative = (name) => {
    document.getElementById(`${name}-negative-count`).value = parseInt(document.getElementById(`${name}-negative-count`).value) + 1;
    document.getElementById(`show-${name}-negative-count`).textContent = document.getElementById(`${name}-negative-count`).value;
}


const sendSearchReviewPosinaga = () => {
...
const changeEndSpaceMessage = (name) => {
    const positiveCount = parseInt(document.getElementById(`${name}-positive-count`).value);
    const negativeCount = parseInt(document.getElementById(`${name}-negative-count`).value);
    const percent = positiveCount + negativeCount !== 0 ? parseInt(positiveCount / (positiveCount + negativeCount) * 100) : 0;
    const spaceMessage = `
        <div class="flex justify-left">
            <p>計測終了</p>(優良率: ${percent}%)
        </div>`;

    document.getElementById(`${name}-spaces-message`).innerHTML = spaceMessage;
}
...
const updateProgressBar = (percent) => {
    const html = `
        <div class="w-full bg-gray-200 rounded-full dark:bg-gray-700 mb-6 mt-6">
